...
class TestAuthRouter:
    ...
    def test_refresh_invalid_token(self, client: TestClient, db_session: Session):
    ...
    def test_get_current_user_invalid_token(self, client: TestClient, db_session: Session):
        """Test get current user with invalid token"""
        headers = {"Authorization": "Bearer invalid_token"}
        response = client.get("/auth/me", headers=headers)
        assert response.status_code == 401
    
    def test_revoke_refresh_token(self, client: TestClient, db_session: Session):
        ...
        # Revoke token
        response = client.post("/auth/revoke-refresh-token", json={"refresh_token": refresh_token})
        assert response.status_code == 200
    ...
    def test_revoke_all_refresh_tokens(self, client: TestClient, db_session: Session):
        ...
        # Revoke all tokens
        headers = {"Authorization": f"Bearer {access_token}"}
        response = client.post("/auth/revoke-all-refresh-tokens", headers=headers)
        assert response.status_code == 200
    ...
    def test_get_user_refresh_tokens(self, client: TestClient, db_session: Session):
    ...
    def test_rate_limiting_registration(self, client: TestClient, db_session: Session):
    ...
    def test_rate_limiting_login(self, client: TestClient, db_session: Session):
