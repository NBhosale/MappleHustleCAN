...
class TestUserRegistrationFlow:
    ...
    def test_provider_onboarding_flow(
            self,
            client: TestClient,
            db_session: Session):
        ...
        response = client.post("/auth/register", json=provider_data)
        assert response.status_code == 201
        ...
        response = client.post(
            "/services/", json=service_data, headers=headers)
        assert response.status_code == 201
        ...
        availability_data = {
            ...
            "end_time": "17:00:00",    # 17:00
            "status": "available"
        }

        response = client.post("/services/availability",
    ...
    def test_service_booking_flow(
        ...
        provider_headers = {
            "Authorization": f"Bearer {
                self._get_provider_token(
                    client, test_provider)}"}
        response = client.post(
            "/services/", json=service_data, headers=provider_headers)
        assert response.status_code == 201
        service = response.json()
        ...
        response = client.get(
            "/search/services?q=cleaning&service_type=house_cleaning")
        assert response.status_code == 200
        ...
        client_headers = {
            "Authorization": f"Bearer {
                self._get_client_token(
                    client, test_user)}"}
        response = client.post(
            "/bookings/", json=booking_data, headers=client_headers)
        assert response.status_code == 201
        ...
        response = client.put(
            f"/bookings/{booking['id']}/accept", headers=provider_headers)
        assert response.status_code == 200

        # 6. Verify booking status
        response = client.get(
            f"/bookings/{booking['id']}", headers=client_headers)
        assert response.status_code == 200
...
class TestMarketplaceFlow:
    ...
    def test_marketplace_flow(
        ...
        client_headers = {
            "Authorization": f"Bearer {
                self._get_client_token(
                    client, test_user)}"}
        response = client.post(
            "/orders/", json=order_data, headers=client_headers)
        assert response.status_code == 201
        order = response.json()
        ...
        response = client.put(
            f"/orders/{order['id']}/process", headers=provider_headers)
        assert response.status_code == 200
        ...
        client_headers = {
            "Authorization": f"Bearer {
                self._get_client_token(
                    client, test_user)}"}
        response = client.post(
            "/messages/", json=message_data, headers=client_headers)
        assert response.status_code == 201
        ...
        provider_headers = {
            "Authorization": f"Bearer {
                self._get_provider_token(
                    client, test_provider)}"}
        response = client.post(
            "/messages/", json=response_data, headers=provider_headers)
        assert response.status_code == 201

        # 3. Client gets conversation
        response = client.get(
            f"/messages/conversation/{test_provider.id}", headers=client_headers)
        assert response.status_code == 200
...
class TestPaymentFlow:
    ...
    def test_payment_flow(
        ...
        client_headers = {
            "Authorization": f"Bearer {
                self._get_client_token(
                    client, test_user)}"}
        response = client.post(
            "/payments/", json=payment_data, headers=client_headers)
        assert response.status_code == 201
        ...
        # 2. Process payment
        response = client.post(
            f"/payments/{payment['id']}/process", headers=client_headers)
        assert response.status_code == 200

        # 3. Verify payment status
        response = client.get(
            f"/payments/{payment['id']}", headers=client_headers)
        assert response.status_code == 200
