...
class MapleHustleUser(HttpUser):
    ...
    def _setup_provider(self):
        ...
        if response.status_code == 201:
            ...
            self._login_provider(provider_data["email"], provider_data["password"])
    ...
    def browse_services(self):
        ...
        if self.user_type in ["client", "provider"]:
            # Search for services
            search_terms = ["cleaning", "dog walking", "lawn maintenance", "house sitting"]
    ...
    def create_service(self):
        ...
        if self.user_type == "provider" and self.token:
            service_types = ["dog_walking", "house_cleaning", "lawn_maintenance", "dog_sitting"]
            ...
            response = self.client.post("/services/", json=service_data, headers=self.headers)
            if response.status_code == 201:
    ...
    def create_item(self):
        ...
        if self.user_type == "provider" and self.token:
            item_names = ["Handmade Dog Collar", "Garden Decor", "Home Accessories", "Pet Supplies"]
            ...
            response = self.client.post("/items/", json=item_data, headers=self.headers)
            if response.status_code == 201:
    ...
    def add_availability(self):
        ...
        if self.user_type == "provider" and self.token:
            ...
            response = self.client.post("/services/availability", json=availability_data, headers=self.headers)
    ...
    def create_booking(self):
        ...
        if self.user_type == "client" and self.token and self.service_id:
            booking_data = {
                "provider_id": str(uuid.uuid4()),  # Random provider ID for testing
            ...
            response = self.client.post("/bookings/", json=booking_data, headers=self.headers)
            if response.status_code == 201:
        ...
        if self.user_type == "client" and self.token and self.item_id:
            order_data = {
                ...
                "shipping_address": {
                    ...
                    "postal_code": f"{random.choice(['M', 'V', 'H', 'T'])}{random.randint(1, 9)}{random.choice(['A', 'B', 'C'])} {random.randint(1, 9)}{random.choice(['A', 'B', 'C'])}{random.randint(1, 9)}"
                }
            }
            
            response = self.client.post("/orders/", json=order_data, headers=self.headers)
            if response.status_code == 201:
    ...
    def send_message(self):
        ...
        if self.token:
            message_data = {
                "recipient_id": str(uuid.uuid4()),  # Random recipient for testing
                "content": f"Test message {uuid.uuid4().hex[:8]}"
            }
            
            response = self.client.post("/messages/", json=message_data, headers=self.headers)
    ...
    def admin_dashboard(self):
        ...
        if self.user_type == "admin" and self.token:
            response = self.client.get("/users/admin/dashboard", headers=self.headers)
            assert response.status_code == 200
    ...
    def security_metrics(self):
        ...
        if self.user_type == "admin" and self.token:
            response = self.client.get("/security/metrics", headers=self.headers)
            assert response.status_code == 200
...
class ClientUser(HttpUser):
    ...
    def search_services(self):
        """Search for services"""
        search_terms = ["cleaning", "dog walking", "lawn maintenance", "house sitting", "errands"]
        search_term = random.choice(search_terms)
    ...
    def search_items(self):
        """Search for items"""
        search_terms = ["handmade", "dog collar", "home decor", "garden", "pet supplies"]
        search_term = random.choice(search_terms)
...
class ProviderUser(HttpUser):
    ...
    def create_service(self):
        """Create a service"""
        if self.token:
            service_types = ["dog_walking", "house_cleaning", "lawn_maintenance", "dog_sitting", "house_sitting", "errands"]
            service_type = random.choice(service_types)
    ...
    def create_item(self):
        ...
        if self.token:
            item_categories = ["Pet Supplies", "Home Decor", "Garden", "Handmade", "Electronics"]
...
class AdminUser(HttpUser):
    ...
    def view_security_events(self):
        ...
        if self.token:
            response = self.client.get("/security/events", headers=self.headers)
            assert response.status_code == 200
