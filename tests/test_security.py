...
class TestCORSSecurity:
    ...
    def test_cors_disallowed_origin(self):
        ...
        # Use a custom client without testclient user-agent to test actual CORS
        # behavior
        from fastapi.testclient import TestClient
        ...
        # Should be blocked due to disallowed origin
...
class TestTrustedHostValidation:
    ...
    def test_untrusted_host_blocked(self):
        ...
        # Use a custom client without testclient user-agent to bypass test
        # bypass
        from fastapi.testclient import TestClient
        ...
        response = test_client.get(
            "/",
            headers={"Host": "malicious-site.com", "User-Agent": "Browser"}
        )
...
class TestRateLimiting:
    ...
    def test_rate_limiting_exceeds_limit(self):
        ...
        if rate_limited_responses:
            ...
            assert "Rate limit exceeded" in rate_limit_response.json()[
                "detail"]
            assert "RATE_LIMIT_EXCEEDED" in rate_limit_response.json()[
                "error_code"]
...
class TestFileUploadSecurity:
    ...
    def test_file_upload_size_limit(self):
        ...
        user_data, tokens = create_user(client, "test@example.com")
    ...
    def test_file_upload_invalid_type(self):
        ...
        # Create a test user and get token
        from tests.factories import create_user
        user_data, tokens = create_user(client, "test2@example.com")
        ...
        assert response.status_code == 400
        assert "File type" in response.json()["message"]

    def test_file_upload_valid_type(self):
        """Test file upload with valid type"""
        # Create a test user and get token
        from tests.factories import create_user
        user_data, tokens = create_user(client, "test3@example.com")

        content = b"fake image data"

        with patch("app.routers.files.save_file") as mock_save:
...
class TestSecurityIntegration:
    ...
    def test_multiple_security_features(self):
        ...
        malicious_request = {
            ...
            "size": "x" * 1000  # Smaller size to avoid URL length limits
        }
