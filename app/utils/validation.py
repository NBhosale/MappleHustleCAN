...
def validate_service_availability_conflicts(
    ...
    if conflicting_slots:
        ...
        raise ValidationError(
            f"Time slot conflicts with existing availability: {
                ', '.join(conflict_times)}")
...
def validate_booking_time_slot(
    ...
    if not availability:
        raise ValidationError(
            "Provider is not available for the requested time slot")
...
def validate_booking_advance_notice(
        start_date: date,
...
def validate_geographic_constraints(
    ...
    same_province_services = ["house_sitting",
                              "lawn_maintenance", "house_cleaning"]
...
def validate_user_verification_status(user_id: str, db: Session) -> None:
    ...
    if user.role == "provider":
        ...
        provider = db.query(Provider).filter(
            Provider.user_id == user_id).first()
...
def validate_booking_request(
    ...
    validate_booking_time_slot(
        db, service_id, start_date_only, start_time_only, end_time_only)
