...
rate_limit_user_actions = create_rate_limit_decorator("200/hour", key_func=get_user_id_for_rate_limit)

# Brute force protection
def check_brute_force_attempts(identifier: str, max_attempts: int = 5, window_minutes: int = 15) -> bool:
...
def record_brute_force_attempt(identifier: str, max_attempts: int = 5, window_minutes: int = 15):
...
def enhanced_rate_limit_exceeded_handler(request: Request, exc: RateLimitExceeded):
    ...
    response_data = {
        ...
        "timestamp": exc.detail.get('timestamp', None) if hasattr(exc, 'detail') else None,
