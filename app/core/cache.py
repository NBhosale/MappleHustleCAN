...
async def cache_user(
        user_id: str, user_data: Dict[str, Any], expire: int = 3600):
    """Cache user data"""
    key = f"user:{user_id}"
    manager = _cache_manager
    if manager:
...
async def cache_service(
        service_id: str, service_data: Dict[str, Any], expire: int = 1800):
    """Cache service data"""
...
async def cache_search_results(
        query: str, results: List[Dict[str, Any]], expire: int = 900):
