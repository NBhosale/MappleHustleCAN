...
class BusinessRuleValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def _validate_content_type(self, request: Request):
        ...
        if request.method in ["POST", "PUT", "PATCH"]:
            # Allow different content types for different endpoints
            path = request.url.path

            # Skip content-type validation for certain endpoints
            if (path.startswith("/auth/login") or
                path.startswith("/auth/refresh") or
                path.startswith("/uploads/") or
                    path.startswith("/files/")):
                # Allow form data for login and file uploads, query params for
                # refresh
                pass
