...
from app.core.logging_context import get_logger
...
class MetricsCollector:
    ...
    def get_metric_stats(
            self, name: str, since: datetime = None) -> Dict[str, float]:
...
class PerformanceMonitor:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        api_metrics = self.collector.get_metric_stats(
            "api_request_duration", since)
        db_metrics = self.collector.get_metric_stats(
            "database_query_duration", since)
        cache_metrics = self.collector.get_metric_stats(
            "cache_operation_duration", since)
            ...
            for endpoint, count in self.request_counts.items():
                ...
                stats[endpoint] = {
                    ...
                    "error_rate_percent": round(
                        (error_count / count * 100) if count > 0 else 0,
                        2),
                    "avg_response_time_ms": round(
                        sum(response_times) / len(response_times) * 1000,
                        2) if response_times else 0,
                    "min_response_time_ms": round(
                        min(response_times) * 1000,
                        2) if response_times else 0,
...
class DatabasePerformanceTracker:
    ...
    def track_connection_pool():
        ...
        try:
            ...
            logger.error(
                "Failed to track database connection pool", error=str(e))
