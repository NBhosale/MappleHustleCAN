...
class SecurityAlertManager:
    ...
    async def send_alert(self, event: SecurityEvent, anomaly: Optional[Dict[str, Any]] = None):
        """Send security alert"""
        ...
        if alert_key in self.last_alerts:
            if (now - self.last_alerts[alert_key]).seconds < self.alert_cooldown:
                return
    ...
    async def _send_email_alert(self, event: SecurityEvent, anomaly: Optional[Dict[str, Any]] = None):
        """Send email alert"""
        try:
            msg = MIMEMultipart()
            msg['From'] = self.email_config.get("from", "security@maplehustlecan.com")
    ...
    async def _send_webhook_alert(self, event: SecurityEvent, anomaly: Optional[Dict[str, Any]] = None):
        """Send webhook alert"""
        try:
            payload = {
    ...
    def _format_alert_message(self, event: SecurityEvent, anomaly: Optional[Dict[str, Any]] = None) -> str:
...
class SecurityMonitor:
    ...
    def record_event(self, event: SecurityEvent):
        ...
        # Log the event
        logger.warning(f"Security event: {event.event_type.value} from {event.client_ip}")
        
        # Check for anomalies
        if self.anomaly_detection_enabled:
            anomalies = self.metrics.detect_anomalies()
            for anomaly in anomalies:
                logger.critical(f"Security anomaly detected: {anomaly['type']}")
                
                # Send alert if enabled
                if self.alerting_enabled:
                    asyncio.create_task(self.alert_manager.send_alert(event, anomaly))
