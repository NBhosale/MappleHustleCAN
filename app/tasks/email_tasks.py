...
def send_password_reset_email(
    ...
    try:
        ...
        reset_url = f"{
            settings.ALLOWED_ORIGINS[0]}/reset-password?token={reset_token}"
    ...
    try:
        ...
        for email_data in email_list:
            try:
                ...
                results.append(
                    {"email": email_data["email"], "status": "success"})

            except Exception as e:
                logger.error(
                    f"Failed to send email to {email_data['email']}: {e}")
                results.append(
                    {"email": email_data["email"], "status": "failed", "error": str(e)})

        success_count = len([r for r in results if r["status"] == "success"])
        logger.info(
            f"Bulk email sent: {success_count}/{len(email_list)} successful")
...
@celery_app.task(bind=True, max_retries=3)
def send_notification_email(self, user_email: str,
                            notification_type: str, data: Dict[str, Any]):
    """Send notification email"""
    try:
        subject = f"MapleHustleCAN Notification: {
            notification_type.replace(
                '_', ' ').title()}"
...
# Task aliases for easier importing
send_welcome_email_task = send_welcome_email
send_password_reset_email_task = send_password_reset_email
# Using notification email for verification
send_email_verification_task = send_notification_email
