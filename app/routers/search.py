...
from app.models.items import Item, ItemCategory
...
def search_services(
    ...
    is_featured: Optional[bool] = Query(None, description="Filter featured services"),
...
def search_available_services(
    ...
    try:
        ...
        if city or province:
            availability_query = availability_query.join(User, Availability.provider_id == User.id)
            if city:
                availability_query = availability_query.filter(User.city.ilike(f"%{city}%"))
            if province:
                availability_query = availability_query.filter(User.province_code == province.upper())
        ...
        if service_type:
            services_query = services_query.filter(Service.type == service_type)
        ...
        raise HTTPException(status_code=500, detail=f"Availability search failed: {str(e)}")
...
def search_items(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Item search failed: {str(e)}")
...
def get_search_suggestions(
    ...
    type: str = Query("all", description="Suggestion type (services, items, providers)"),
    ...
    try:
        ...
        if type in ["all", "services"]:
            ...
            suggestions.extend([{"text": s.title, "type": "service"} for s in service_suggestions])
        ...
        if type in ["all", "items"]:
            ...
            suggestions.extend([{"text": s.name, "type": "item"} for s in item_suggestions])
        ...
        if type in ["all", "providers"]:
            ...
            suggestions.extend([{"text": s.name, "type": "provider"} for s in provider_suggestions])
